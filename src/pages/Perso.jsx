...
import { PERSO_CATEGORIES } from '../hooks/useCategories'
...
export default function Perso() {
  ...
  const [form, setForm] = useState({ direction: 'expense', amount: '', category: PERSO_CATEGORIES[0], description: '', date: new Date().toISOString().slice(0, 10) })

  const { data: transactions = [], isLoading } = useTransactions({ type: 'perso', month })
  ...
  function handleSubmit(e) {
    e.preventDefault()
    addTx.mutate({ ...form, type: 'perso', amount: parseFloat(form.amount) }, {
      onSuccess: () => { setShowForm(false); setForm({ direction: 'expense', amount: '', category: PERSO_CATEGORIES[0], description: '', date: new Date().toISOString().slice(0, 10) }) }
    })
  }
    ...
    <div className="page">
      ...
      {showForm && (
        <form className="card" onSubmit={handleSubmit} style={{ marginBottom: '1.5rem', display: 'grid', gap: '0.75rem' }}>
          <p style={{ fontWeight: 600 }}>Nouvelle transaction perso</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
              ...
              <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: 'var(--radius)', padding: '0.4rem' }}>
                {PERSO_CATEGORIES.map(c => <option key={c}>{c}</option>)}
              </select>
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
